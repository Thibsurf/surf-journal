// ════════════════════════════════════════════════════════════════════════════
// enso.js — Extrait de previsions.html (AUDIT-previsions.md T18, chantier 2)
//
// Script CLASSIQUE (pas de module ES) : toutes les déclarations top-level sont
// des globals, exactement comme quand ce code vivait inline dans previsions.html —
// aucun changement de sémantique, seul l'emplacement du fichier change. Chargé en
// <script defer>, donc après le script principal (previsions.html s'appuie sur ça :
// ensoLoadData()/ensoRender() ne sont appelés que via showTab('enso'), déclenché par
// un clic utilisateur — jamais au chargement, donc jamais avant que ce fichier soit prêt).
// ════════════════════════════════════════════════════════════════════════════
...
function ensoRender() {
  ...
  Plotly.react('enso-hist', [
    ...
    // type:'linear' explicite : les graphes temporels ci-dessus (Chart 1/2) font
    // écrire type:'date' par Plotly DANS l'objet LAYOUT.xaxis partagé (Plotly mute
    // le layout qu'on lui passe). Sans ça, l'histogramme héritait de l'axe date et
    // affichait les anomalies (~-2 à +3) comme des millisecondes → « 11:00:00 ».
    xaxis: Object.assign({}, LAYOUT.xaxis, { type: 'linear', autorange: true, title: { text: 'Anomalie ONI (°C)', standoff: 8 } }),
    yaxis: Object.assign({}, LAYOUT.yaxis, { type: 'linear', autorange: true, title: 'Nb mois' }),
  }), CFG);
